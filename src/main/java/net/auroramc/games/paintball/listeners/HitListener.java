    ...
    public void onDamage(PlayerDamageByPlayerEvent e) {
        if (e.getDamager() instanceof Snowball) {
            ...
            if (snowball.getShooter() instanceof ArmorStand) {
                ...
                shooter = ServerAPI.getPlayer((Player) snowball.getShooter());
            ...
            if (shooter != null) {
                ...
                for (AuroraMCServerPlayer player1 : ServerAPI.getPlayers()) {
                    ...
                    player1.sendMessage(TextFormatter.pluginMessage("Kill", killMessage.onKill(player1, turret.getOwner(), player, WordUtils.capitalizeFully(WordUtils.capitalizeFully(turret.getArmorStand().getType().name().replace("_", " "))), KillMessage.KillReason.PAINTBALL, EngineAPI.getActiveGameInfo().getId())));
                }
                EngineAPI.getActiveGame().getGameSession().log(new GameSession.GameLogEntry(GameSession.GameEvent.DEATH, new JSONObject().put("player", player.getByDisguiseName()).put("killer", turret.getOwner().getByDisguiseName()).put("final", false)));
            } else {
